...
from attend_and_excite_sdxl.utils import (
...
class StableDiffusionXLAttendAndExcitePipeline(StableDiffusionXLPipeline):
    vae: AutoencoderKL
    ...
    def _aggregate_and_get_max_attention_per_token(
        ...
        # attn_maps_pl = []
        # for idx in indices:
        #     attn_map = attention_maps[:, :, idx].clone().detach()
        #     attn_map = (attn_map - attn_map.min()) / (attn_map.max() - attn_map.min())
        #     attn_map = (attn_map.detach().cpu().float().numpy() * 255).astype(np.uint8)

        #     attn_maps_pl.append(Image.fromarray(attn_map))

        # save_dir = pathlib.Path(__file__).resolve().parents[2] / "generated" / "sdxl-ae"
        # save_dir.mkdir(parents=True, exist_ok=True)

        # saved_files = sorted(
        #     [f for f in save_dir.iterdir() if f.suffix == ".png"],
        ...
        #     else save_dir / f"{int(saved_files[-1].stem) + 1}.png"
        # )

        # image = make_image_grid(images=attn_maps_pl, rows=1, cols=len(attn_maps_pl))
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # ==============================
        # Settings for Attend-and-Excite
        # ==============================
        ...
        # ==============================
        # End of settings for Attend-and-Excite
        # ==============================
            ...
            for i, t in enumerate(timesteps):
                ...
                with torch.enable_grad():
                    ...
                    for (
                        ...
                        latent = latent[None, :, :, :]
                        prompt_embed_ae = prompt_embed_ae[None, :, :]
                        add_text_embed_ae = add_text_embed_ae[None, :]
                        add_time_id_ae = add_time_id_ae[None, :]

                        # predict the noise residual
                        added_cond_kwargs = {
                            "text_embeds": add_text_embed_ae,
                            "time_ids": add_time_id_ae,
                        }
                        if (
                        ...
                        self.unet(
                            latent,
                            t,
                            encoder_hidden_states=prompt_embed_ae,
                            timestep_cond=timestep_cond,
                            cross_attention_kwargs=self.cross_attention_kwargs,
                            added_cond_kwargs=added_cond_kwargs,
                        ).sample
                        ...
                        if i in thresholds.keys() and loss > 1.0 - thresholds[i]:
                            loss, latent, max_attention_per_index = (
                                self._perform_iterative_refinement_step(
                                    ...
                                    text_embeddings=prompt_embed_ae,
